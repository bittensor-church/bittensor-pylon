...
def test_get_latest_prices_success(pact: Pact, get_prices_response_matcher: dict, pylon_client_factory):
    (
    ...
    with pact.serve() as srv:
        ...
        with client:
            response = client.unstable.open_access.get_latest_prices()

    assert response == build_prices()


def test_get_prices_success(pact: Pact, get_prices_response_matcher: dict, pylon_client_factory):
    (
    ...
    with pact.serve() as srv:
        client = pylon_client_factory(str(srv.url))
        with client:
            response = client.unstable.open_access.get_prices(block_number=BlockNumber(BLOCK_NUMBER))
