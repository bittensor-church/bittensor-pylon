    ...
    with pact.serve() as srv:
        ...
        async with client:
            with pytest.raises(PylonBadGateway, match=r"^Bad gateway \(HTTP 502\): Block 123 data is unavailable$"):
                await client.unstable.open_access.get_neurons(netuid=NetUid(1), block_number=BlockNumber(123))
    ...
    with pact.serve() as srv:
        ...
        async with client:
            with pytest.raises(
                ...
                await client.unstable.open_access.get_latest_neurons(netuid=NetUid(1))
