...
@pytest.mark.asyncio
async def test_get_own_revealed_commitments_success(
    pact: Pact, get_own_revealed_commitments_response_matcher: dict, pylon_client_factory
):
    (
        pact.upon_receiving("an identity request for own revealed commitments")
        .given("own revealed commitments exist", identity_name=IDENTITY_NAME, netuid=NETUID, hotkey=HOTKEY_1)
        .with_request(
            "GET", f"/api/_unstable/identity/{IDENTITY_NAME}/subnet/{NETUID}/block/latest/commitments/revealed/self"
        )
    ...
    with pact.serve() as srv:
        ...
        async with client:
            response = await client.unstable.identity.get_own_revealed_commitments()
