...
class TestAsyncOpenAccessGetLatestEvmAssociations(OpenAccessEndpointTest):
    endpoint = EndpointUnstable.LATEST_EVM_ASSOCIATIONS
    route_params = {"netuid": 1}
    http_method = HTTPMethod.GET

    async def make_endpoint_call(self, client):
        return await client.unstable.open_access.get_latest_evm_associations(netuid=NetUid(1))
    ...
    async def test_success_with_no_evm_associations(self, pylon_client, service_mock, route_mock, block):
        ...
        async with pylon_client:
            response = await pylon_client.unstable.open_access.get_latest_evm_associations(netuid=NetUid(1))
