...
class TestAsyncOpenAccessGetCommitmentsV1(OpenAccessEndpointTest):
    endpoint = EndpointV1.LATEST_COMMITMENTS
    route_params = {"netuid": 1}
    http_method = HTTPMethod.GET

    async def make_endpoint_call(self, client):
        return await client.v1.open_access.get_commitments(netuid=NetUid(1))
    ...
    async def test_success_with_empty_commitments(self, pylon_client, service_mock, route_mock, block):
        ...
        async with pylon_client:
            response = await pylon_client.v1.open_access.get_commitments(netuid=NetUid(1))
