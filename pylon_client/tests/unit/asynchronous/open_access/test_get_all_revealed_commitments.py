...
class TestAsyncOpenAccessGetAllRevealedCommitments(OpenAccessEndpointTest):
    endpoint = EndpointUnstable.LATEST_COMMITMENTS_REVEALED
    route_params = {"netuid": 1}
    http_method = HTTPMethod.GET

    async def make_endpoint_call(self, client):
        return await client.unstable.open_access.get_all_revealed_commitments(netuid=NetUid(1))
