...
class TestOpenAccessGetLatestPrice(OpenAccessEndpointTest):
    endpoint = EndpointUnstable.SUBNET_LATEST_PRICE
    route_params = {"netuid": 1}
    http_method = HTTPMethod.GET

    async def make_endpoint_call(self, client):
        return await client.unstable.open_access.get_latest_price(netuid=NetUid(1))
...
class TestOpenAccessGetPrice(OpenAccessEndpointTest):
    endpoint = EndpointUnstable.SUBNET_PRICE
    route_params = {"netuid": 1, "block_number": 500}
    http_method = HTTPMethod.GET

    async def make_endpoint_call(self, client):
        return await client.unstable.open_access.get_price(netuid=NetUid(1), block_number=BlockNumber(500))
