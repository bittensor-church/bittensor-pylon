...
def test_retries_with_refreshed_netuid_on_308(
    sync_identity_client: PylonClient,
    service_mock,
    ...
    with sync_identity_client:
        result = sync_identity_client.unstable.identity.get_latest_neurons()
...
def test_raises_on_308_after_refresh(
    sync_identity_client: PylonClient,
    service_mock,
):
    """
    ...
    with sync_identity_client:
        with pytest.raises(PylonNetuidMismatch, match=r"Netuid mismatch \(HTTP 308\)"):
            sync_identity_client.unstable.identity.get_latest_neurons()
