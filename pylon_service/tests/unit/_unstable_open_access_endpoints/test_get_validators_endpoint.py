...
async def test_unstable_open_access_get_validators_returns_only_validator_permit_neurons_sorted_by_total_stake_desc(
    ...
    response = await open_access_test_client.get(
        f"/api/_unstable/openaccess/subnet/{VALIDATORS_NETUID}/block/321/validators"
    )
...
async def test_unstable_open_access_get_latest_validators_returns_only_validator_permit_neurons_sorted_by_total_stake_desc(
    ...
    response = await open_access_test_client.get(
        f"/api/_unstable/openaccess/subnet/{VALIDATORS_NETUID}/block/latest/validators"
    )
...
async def test_get_validators_open_access_block_not_found(
    ...
    async with mock_bt_client_factory() as mock_client:
        async with mock_client.mock_behavior(
            ...
            response = await open_access_test_client.get("/api/_unstable/openaccess/subnet/1/block/999999/validators")
...
async def test_get_validators_open_access_invalid_block_number_type(
    ...
    response = await open_access_test_client.get(
        f"/api/_unstable/openaccess/subnet/1/block/{invalid_block_number}/validators"
    )
