...
async def test_get_neurons_identity_invalid_block_number_type(
    ...
    async with identity_test_client_factory("sn1") as client:
        response = await client.get(
            f"/api/_unstable/identity/sn1/subnet/1/block/{invalid_block_number}/neurons",
        )
...
@pytest.mark.asyncio
async def test_unstable_identity_get_neurons_returns_block_neurons(identity_test_client_factory, snapshot_json):
    async with identity_test_client_factory("sn1") as client:
        response = await client.get(
            "/api/_unstable/identity/sn1/subnet/1/block/123/neurons",
        )
...
@pytest.mark.asyncio
async def test_unstable_identity_get_latest_neurons_returns_latest_neurons(identity_test_client_factory, snapshot_json):
    async with identity_test_client_factory("sn1") as client:
        response = await client.get(
...
@pytest.mark.asyncio
async def test_unstable_identity_any_identity_scoped_endpoint_unknown_identity_returns_404(
    identity_test_client_factory, snapshot_json
):
    async with identity_test_client_factory("sn1") as client:
        response = await client.get(
            "/api/_unstable/identity/unknown/subnet/1/block/latest/neurons",
        )
