        ...
        async with mock_client.mock_behavior(
            ...
            async with identity_test_client_factory("sn1") as client:
                response = await client.get(
                    "/api/_unstable/identity/sn1/subnet/1/block/latest/certificates/self",
                )
    ...
    async with mock_bt_client_factory("sn2") as mock_client:
        async with mock_client.mock_behavior(
            ...
            async with identity_test_client_factory("sn2") as client:
                response = await client.get(
                    "/api/_unstable/identity/sn2/subnet/2/block/latest/certificates/self",
                )
...
@pytest.mark.asyncio
async def test_unstable_identity_get_own_certificate_unknown_identity_returns_404(
    identity_test_client_factory, snapshot_json
):
    async with identity_test_client_factory("sn1") as client:
        response = await client.get(
            "/api/_unstable/identity/unknown/subnet/1/block/latest/certificates/self",
        )
