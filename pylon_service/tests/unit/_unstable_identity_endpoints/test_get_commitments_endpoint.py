...
@pytest.mark.asyncio
async def test_unstable_identity_get_commitments_returns_hex_and_timelock_variants(
    identity_test_client_factory, snapshot_json
):
    async with identity_test_client_factory("sn25") as client:
        response = await client.get(
            f"/api/_unstable/identity/sn25/subnet/{COMMITMENTS_MIXED_NETUID}/block/latest/commitments"
        )
