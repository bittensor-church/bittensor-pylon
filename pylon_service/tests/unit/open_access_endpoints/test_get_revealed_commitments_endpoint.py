"""
Tests for the GET /subnet/{netuid}/block/latest/commitments/revealed/{hotkey} endpoint.
"""
...
@pytest.mark.asyncio
async def test_v1_open_access_get_revealed_commitments_by_hotkey_returns_list(
    ...
    async with open_access_mock_bt_client.mock_behavior(get_revealed_commitments=[resolve_revealed_commitments]):
        response = await test_client.get(
            f"/api/v1/subnet/{REVEALED_COMMITMENTS_NETUID}/block/latest/commitments/revealed/hotkey1"
        )
...
@pytest.mark.asyncio
async def test_v1_open_access_get_revealed_commitments_by_hotkey_returns_404_when_missing(
    test_client: AsyncTestClient, open_access_mock_bt_client: MockBittensorContact, snapshot_json
):
