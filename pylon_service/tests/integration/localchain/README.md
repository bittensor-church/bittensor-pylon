...
Four deterministic dev accounts are available (defined in `dev_accounts.py`). Their wallet
files are generated by the native v11 SDK. The `dev_wallets` fixture creates a session
directory with `tmp_path_factory` and configures test identities to use it. Standalone chain
preparation scripts use the same generator with a process-owned temporary directory:
