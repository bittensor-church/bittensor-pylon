...
Pylon accepts both legacy wallet keyfiles and SR25519 wallets generated by the Bittensor
10.5 and 11 CLIs, without converting or rewriting them. Wallet loading uses
`bittensor.wallet` from Bittensor 11, together with turbobt 1.4.0 or newer. Neither package
requires the standalone `bittensor-wallet` dependency or a dependency override. See
[wallet compatibility fixtures](tests/WALLETS.md) for the tested CLI versions and
end-to-end test instructions.

Release prerequisite: publish the corresponding turbobt 1.4.0 migration first, then run
`uv lock --upgrade-package turbobt` here and include the refreshed lockfile in the Pylon
release. The checked-in lockfile remains at the previous release until that package is
available on PyPI; `uv sync --locked` will reject it until it is refreshed. For pre-release
validation, build a candidate wheel in the turbobt checkout with
`PDM_BUILD_SCM_VERSION=1.4.0 uv build --wheel --out-dir /tmp/turbobt-candidate`, then run
`UV_FIND_LINKS=/tmp/turbobt-candidate uv run nox -s test` here. Keep the resulting local-wheel
lockfile out of the release; it must be regenerated from PyPI after publishing turbobt.
