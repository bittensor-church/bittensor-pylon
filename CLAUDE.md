...
The project has two independent products with separate release workflows. Versions
are determined from git tags using `hatch-vcs`. Changelogs are generated by
[commitizen](https://commitizen-tools.github.io/commitizen/) (added as a dev
dependency in each package) and filtered by the `Impacts:` footer in commit messages
via package-specific entry points from the in-repo `pylon_cz` plugin
(`tools/pylon_cz/`).
...
comma-separated (`Impacts: client, service`). Commits with `Impacts: commons` are
included in BOTH changelogs because `pylon_commons` is shared. Commits without an
`Impacts:` footer are excluded from every changelog (intended for repo-infrastructure
commits like CI tweaks).

The header itself follows standard Conventional Commits: `<type>[!]: <subject>` —
types are `feat`, `fix`, `refactor`, `perf`, `build`, `ci`, `docs`, `style`, `test`,
`chore`, `revert`. Use `!` for breaking changes (e.g. `feat!: ...`).

A pre-commit hook validates the header format. Install once per clone:
...
`nox -s release` will:
- pull `origin/master` first when run from `master` (otherwise it warns before continuing),
- find the latest `client-v*` tag (ignoring `service-v*` and `common-v*` tags),
- collect commits since that tag whose `Impacts:` footer mentions `client` or
  `commons`,
- decide MAJOR/MINOR/PATCH from commit types (`feat!` → MAJOR, `feat` → MINOR,
  `fix`/`refactor`/`perf` → PATCH),
- show `cz bump --dry-run --increment ...`,
- ask for confirmation,
- update `pylon_client/CHANGELOG.md`,
- create a `chore: bump pylon_client ...` commit,
- create an annotated tag `client-vX.Y.Z`,
- push the release commit and tag.

The push triggers the existing `cd-pypi-client.yml` workflow to publish to PyPI.

#### Service Release (Docker Hub)

Identical, but in `pylon_service/`:
...
This creates a `service-vX.Y.Z` tag, triggering `cd-docker.yml` to publish to Docker Hub.
