...
from pylon.service.bittensor.cache.recent import RecentDataConfig, RecentDataProvider
from pylon.service.bittensor.cache.settings import settings
...
async def recent_data_provider_dep(state: State) -> RecentDataProvider:
    config = RecentDataConfig(
        recent_neurons_soft_limit=settings.recent_neurons_soft_limit,
        recent_neurons_hard_limit=settings.recent_neurons_hard_limit,
    )
    return RecentDataProvider(config, state.cache_manager)
